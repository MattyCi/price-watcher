...
/**
 * Determines whether the current executing user is registered or not.
 * It the user is a guest, either retrieve or create a new record in the
 * users table depending on the cookies that exist from the client.
 * Inserts the current Reguser user into the value stack so that it can be
...
public class UserValidatorInterceptor implements Interceptor {
	...
	public String intercept(ActionInvocation actionInvocation) throws Exception {
		...
		/* determine whether this is a guest or registered user
		   if guest user, decide if we need a new guest user record or not */
		if (shiroUser.getPrincipal() == null) {
			...
			guestUserIDFromCookie = UserUtils.retrieveGuestUserIDFromCookies(ServletActionContext.getRequest().getCookies());
